    ...
    >
      <Group align="center" spacing="md" mb="md">
        ...
        <Stack spacing={0}>
          ...
          <Text size="sm" color="neutral.7">{cv.personalInfo.title}</Text>
        </Stack>
      </Group>
      <Divider mb="sm" />
      {cv.personalInfo.description && (
        <Text size="sm" color="neutral.8" mb="md">{cv.personalInfo.description}</Text>
      )}
      <Divider mb="sm" />
        ...
        <>
          ...
          <Group spacing={6} mb="md" style={{ flexWrap: 'wrap' }}>
            {cv.skills.filter(Boolean).map((s, i) => (
              <Badge key={i} color="neutral" variant="light" size="xs">{s}</Badge>
            ))}
          </Group>
        </>
      )}
        ...
        <>
          ...
          <Group spacing={6} mb="md" style={{ flexWrap: 'wrap' }}>
            {cv.languages.filter(Boolean).map((l, i) => (
              <Badge key={i} color="neutral" variant="outline" size="xs">{l}</Badge>
            ))}
          </Group>
        </>
      )}
        ...
        <>
          ...
          <Group spacing={6} mb="md" style={{ flexWrap: 'wrap' }}>
            {cv.hobbies.filter(Boolean).map((h, i) => (
              <Badge key={i} color="neutral" variant="outline" size="xs">{h}</Badge>
            ))}
          </Group>
        </>
      )}
